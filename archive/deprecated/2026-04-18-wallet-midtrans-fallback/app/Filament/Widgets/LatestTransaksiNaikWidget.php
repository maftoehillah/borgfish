...
class LatestTransaksiNaikWidget extends BaseWidget
{
    ...
    protected static ?string $heading = 'Transaksi Lelang Naik Terbaru';
    ...
    public function table(Table $table): Table
    {
        return $table
            ->query(
                Transaksi::query()
                    ->with(['ikan', 'pemenang'])
                    ->whereHas('ikan', fn ($query) => $query->where('tipe_lelang', 'naik'))
                    ->latest()
                    ->limit(8)
            )
            ->columns([
